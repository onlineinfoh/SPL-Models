...
| File | Added | Why |
|---|---|---|
| `README.md` | Baseline intensity scaling stated per model, and the nnU-Net `CTNormalization` contrast | The supplement describes 1st/99th percentile clipping for both baselines. No such clipping exists in either path. The two baselines also differ from each other: U-Net divides by the per-image maximum only (`benchmarking/unet_benchmark.py:137-138`), while DeepLabv3+ does a true min-max rescale then applies ImageNet statistics (`benchmarking/deeplab_benchmark.py:126-129`, `DeepLabV3Plus-Pytorch/datasets/lung_ct.py:91-93`). Recording what each does prevents the supplement's claim being read back out of this repository. |
